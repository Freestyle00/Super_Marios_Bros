...
namespace Super_Marios_Bros.Entities
{
	public partial class Turtle
	{
		...
		void TurtleHiddenDoingThings()
		{
			if (GotHit)
			{
				this.GroundMovement = PlatformerValuesStatic["InShell"]; //this is harder than i originally thought (as always)}
			}
		}
		//public void TurtleKickStart(string Dir) // i think i should just let it be TURTLE CAN'T BE STOPPED NOW
		//{
		//	if (Dir == "R")
		//	{
		//		this.Velocity.X = 80;
